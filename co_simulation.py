...
import importlib
importlib.reload(TMS_operation)
...
def get_veh(vnum):
    # Vehicle database sourced from NREL FASTSim (https://github.com/NREL/fastsim)
    # Citation: National Renewable Energy Laboratory. FASTSim: Future Automotive Systems Technology Simulator.
...
def sim_drive_sub( cyc , veh , initSoc):
    ...
    mrt_val = np.zeros(steps)
    cabTwindshield = np.zeros(steps)
    cabTrear = np.zeros(steps)
    cabTside_left = np.zeros(steps)
    cabTside_right = np.zeros(steps)
    cabTroof_ext = np.zeros(steps)
    cabTroof_int = np.zeros(steps)
    cabTdoor_left_ext = np.zeros(steps)
    cabTdoor_left_int = np.zeros(steps)
    cabTdoor_right_ext = np.zeros(steps)
    cabTdoor_right_int = np.zeros(steps)
    cabTdashboard = np.zeros(steps)
    cabTseats = np.zeros(steps)
    essT   = np.zeros(steps)
    mcT       = np.zeros(steps)
    T_airsupply          = np.zeros(steps)
    essT_coolin      = np.zeros(steps)
    mcT_coolin = np.zeros(steps)
    mcT_coolout = np.zeros(steps)
    Te = np.zeros(steps)
    plowrfg = np.zeros(steps)
    Tc = np.zeros(steps)
    phighrfg = np.zeros(steps)
    comphrfgout      = np.zeros(steps)
    solar_in = np.zeros(steps)
    q_cab = np.zeros(steps)
    q_occ = np.zeros(steps)
    batHeat = np.zeros(steps)
    q_bat = np.zeros(steps)
    q_motor      = np.zeros(steps)
    Q_act_evap = np.zeros(steps)
    Q_act_cond = np.zeros(steps)
    m_dot    = np.zeros(steps)
    ...
    mcmdotcool = np.zeros(steps)
    essradmdotair = np.zeros(steps)
    mcradmdotair = np.zeros(steps)
    sumPTMS = np.zeros(steps)
    rfgPcomp       = np.zeros(steps)
    cabPfanair = np.zeros(steps)
    evapPfanair   = np.zeros(steps)
    condPfanair = np.zeros(steps)
    esscoolPpump = np.zeros(steps)
    mccoolPpump     = np.zeros(steps)
    essradPfanair = np.zeros(steps)
    mcradPfanair = np.zeros(steps)
    e_cabin = np.zeros(steps)
    i_term_cabin = np.zeros(steps)
    e_battery = np.zeros(steps)
    i_term_battery      = np.zeros(steps)
    e_motor = np.zeros(steps)
    i_term_motor  = np.zeros(steps)
    EER = np.zeros(steps)
    operation_mode = np.zeros(steps)

    Tamb = veh['Tamb']
    n_passenger = veh['n_passenger']
    dni = veh['dni']
    dhi = veh['dhi']
    time_of_day = veh['time_of_day']
    day_of_year = veh['day_of_year']
    lat = veh['lat']
    lon = veh['lon']
    veh_heading = veh['veh_heading']

    cabTair[0] = Tamb
    mrt_val[0] = cabTair[0]
    cabTwindshield[0] = Tamb
    cabTrear[0] = Tamb
    cabTside_left[0] = Tamb
    cabTside_right[0] = Tamb
    cabTroof_ext[0] = Tamb
    cabTroof_int[0] = Tamb
    cabTdoor_left_ext[0] = Tamb
    cabTdoor_left_int[0] = Tamb
    cabTdoor_right_ext[0] = Tamb
    cabTdoor_right_int[0] = Tamb
    cabTdashboard[0] = Tamb
    cabTseats[0] = Tamb
    essT[0] = Tamb
    mcT[0] = Tamb
    T_airsupply[0] = Tamb
    ...
    # 在循环外预实例化TMS模式对象（每次仿真只创建一次，而非每个时间步重复创建）
    _glass_preset = veh.get('glass_preset', 'normal_glass')
    _glass_lib    = veh.get('glass_lib', None)
    _AC           = TMS_operation.AC_mode(_glass_preset, _glass_lib)
    _HP           = TMS_operation.HP_mode(_glass_preset, _glass_lib)
    _Ventilation  = TMS_operation.Ventilation_mode(_glass_preset, _glass_lib)
    ...
    for i in range(1,len(cycSecs)):
        ...
        if veh['noElecAux']=='TRUE':
            ...
            if veh['TMS'] == 'on':
                auxInKw[i] = veh['auxKw'] + sumPTMS[i-1]/1e3
            else:
                auxInKw[i] = veh['auxKw']
...
#                 fcKwInAch[i] = fcKwOutAch[i]/(veh['fcEffArray'][max(1,np.argmax(veh['fcKwOutArray']>min(fcKwOutAch[i],veh['fcMaxOutkW']-0.001))-1)])
        ...
        ### run the TMS simulation
        T_cabin_ref, essT_ref, mcT_ref = TMS_operation.get_seasonal_T_ref(Tamb)
        operation_mode[i], hp_lock = TMS_operation.get_operation_mode(cabTair[i-1], Tamb, T_cabin_ref, hp_lock, operation_mode[i-1])

        if operation_mode[i] == -1:
            AC = _AC

            #main TMS simulation
            cabTair[i], mrt_val[i], cabTwindshield[i], cabTrear[i], cabTside_left[i], cabTside_right[i], \
            cabTroof_ext[i], cabTroof_int[i], cabTdoor_left_ext[i], cabTdoor_left_int[i], cabTdoor_right_ext[i], cabTdoor_right_int[i], cabTdashboard[i], cabTseats[i],\
            essT[i], mcT[i], T_airsupply[i], essT_coolin[i], mcT_coolin[i], mcT_coolout[i], \
            Te[i], plowrfg[i], Tc[i], phighrfg[i], comphrfgout[i], \
            solar_in[i], q_cab[i], q_occ[i], batHeat[i], q_bat[i], q_motor[i], Q_act_evap[i], Q_act_cond[i], \
            m_dot[i], rfgNcomp[i], cabmdotair[i], condmdotair[i], essmdotcool[i], mcmdotcool[i], mcradmdotair[i], \
            sumPTMS[i], rfgPcomp[i], cabPfanair[i], condPfanair[i], esscoolPpump[i], mccoolPpump[i], mcradPfanair[i], EER[i],\
            e_cabin[i], i_term_cabin[i], e_battery[i], i_term_battery[i], e_motor[i], i_term_motor[i] = AC.simulation(essKwOutAch[i-1], mcElecKwInAch[i-1], mcMechKwOutAch[i-1], mphAch[i-1], n_passenger, \
                   Tamb, dni, dhi, time_of_day, day_of_year, lat, lon, veh_heading, \
                   cabTair[i-1], cabTwindshield[i-1], cabTrear[i-1], cabTside_left[i-1], cabTside_right[i-1], \
                   cabTroof_ext[i-1], cabTroof_int[i-1], cabTdoor_left_ext[i-1], cabTdoor_left_int[i-1], cabTdoor_right_ext[i-1], cabTdoor_right_int[i-1], cabTdashboard[i-1], cabTseats[i-1], \
                   essT[i-1], mcT[i-1], T_airsupply[i-1], essT_coolin[i-1], mcT_coolin[i-1], e_cabin[i-1], i_term_cabin[i-1], e_battery[i-1], i_term_battery[i-1], e_motor[i-1], i_term_motor[i-1])
        
        elif operation_mode[i] == 1:
            HP = _HP
            ...
            Te[i], plowrfg[i], Tc[i], phighrfg[i], comphrfgout[i], \
            solar_in[i], q_cab[i], q_occ[i], batHeat[i], q_bat[i], q_motor[i], Q_act_evap[i], Q_act_cond[i], \
            m_dot[i], rfgNcomp[i], cabmdotair[i], evapmdotair[i], essmdotcool[i], mcmdotcool[i], mcradmdotair[i], essradmdotair[i], \
            sumPTMS[i], rfgPcomp[i], cabPfanair[i], evapPfanair[i], esscoolPpump[i], mccoolPpump[i], mcradPfanair[i], essradPfanair[i], EER[i],\
            e_cabin[i], i_term_cabin[i], e_battery[i], i_term_battery[i], e_motor[i], i_term_motor[i] = HP.simulation(essKwOutAch[i-1], mcElecKwInAch[i-1], mcMechKwOutAch[i-1], mphAch[i-1], n_passenger, \
                   Tamb, dni, dhi, time_of_day, day_of_year, lat, lon, veh_heading, \
                   cabTair[i-1], cabTwindshield[i-1], cabTrear[i-1], cabTside_left[i-1], cabTside_right[i-1], \
                   cabTroof_ext[i-1], cabTroof_int[i-1], cabTdoor_left_ext[i-1], cabTdoor_left_int[i-1], cabTdoor_right_ext[i-1], cabTdoor_right_int[i-1], cabTdashboard[i-1], cabTseats[i-1], \
                   essT[i-1], mcT[i-1], T_airsupply[i-1], essT_coolin[i-1], mcT_coolin[i-1], e_cabin[i-1], i_term_cabin[i-1], e_battery[i-1], i_term_battery[i-1], e_motor[i-1], i_term_motor[i-1])
            ...
            Te[i], plowrfg[i], Tc[i], phighrfg[i], comphrfgout[i], \
            solar_in[i], q_cab[i], q_occ[i], batHeat[i], q_bat[i], q_motor[i], Q_act_evap[i], Q_act_cond[i], \
            m_dot[i], rfgNcomp[i], cabmdotair[i], condmdotair[i], essmdotcool[i], mcmdotcool[i], mcradmdotair[i], essradmdotair[i], \
            sumPTMS[i], rfgPcomp[i], cabPfanair[i], condPfanair[i], esscoolPpump[i], mccoolPpump[i], mcradPfanair[i], essradPfanair[i], EER[i],\
            e_cabin[i], i_term_cabin[i], e_battery[i], i_term_battery[i], e_motor[i], i_term_motor[i] = Ventilation.simulation(essKwOutAch[i-1], mcElecKwInAch[i-1], mcMechKwOutAch[i-1], mphAch[i-1], n_passenger, \
                   Tamb, dni, dhi, time_of_day, day_of_year, lat, lon, veh_heading, \
                   cabTair[i-1], cabTwindshield[i-1], cabTrear[i-1], cabTside_left[i-1], cabTside_right[i-1], \
                   cabTroof_ext[i-1], cabTroof_int[i-1], cabTdoor_left_ext[i-1], cabTdoor_left_int[i-1], cabTdoor_right_ext[i-1], cabTdoor_right_int[i-1], cabTdashboard[i-1], cabTseats[i-1], \
                   essT[i-1], mcT[i-1], T_airsupply[i-1], essT_coolin[i-1], mcT_coolin[i-1], e_cabin[i-1], i_term_cabin[i-1], e_battery[i-1], i_term_battery[i-1], e_motor[i-1], i_term_motor[i-1])
    ...
    output['essLossKw']=np.array(essLossKw)
    output['essCurKwh']=np.array(essCurKwh)
...
#     evapPfanair[i]/1e3
    ...
    #output TMS
    output['cabTair'] = np.array(cabTair) - 273.15
    output['mrt_val'] = np.array(mrt_val) - 273.15
    output['cabTwindshield'] = np.array(cabTwindshield) - 273.15
    output['cabTrear'] = np.array(cabTrear) - 273.15
    output['cabTside_left'] = np.array(cabTside_left) - 273.15
    output['cabTside_right'] = np.array(cabTside_right) - 273.15
    output['cabTroof_ext'] = np.array(cabTroof_ext) - 273.15
    output['cabTroof_int'] = np.array(cabTroof_int) - 273.15
    output['cabTdoor_left_ext'] = np.array(cabTdoor_left_ext) - 273.15
    output['cabTdoor_left_int'] = np.array(cabTdoor_left_int) - 273.15
    output['cabTdoor_right_ext'] = np.array(cabTdoor_right_ext) - 273.15
    output['cabTdoor_right_int'] = np.array(cabTdoor_right_int) - 273.15
    output['cabTdashboard'] = np.array(cabTdashboard) - 273.15
    output['cabTseats'] = np.array(cabTseats) - 273.15
    output['essT'] = np.array(essT) - 273.15
    output['mcT'] = np.array(mcT) - 273.15
    output['T_airsupply'] = np.array(T_airsupply) - 273.15
    output['essT_coolin'] = np.array(essT_coolin) - 273.15
    output['mcT_coolin'] = np.array(mcT_coolin) - 273.15
    output['mcT_coolout'] = np.array(mcT_coolout) - 273.15
    output['Te'] = np.array(Te) - 273.15
    output['plowrfg'] = np.array(plowrfg)
    output['Tc'] = np.array(Tc) - 273.15
    output['phighrfg'] = np.array(phighrfg)
    output['comphrfgout'] = np.array(comphrfgout)

    output['solar_in'] = np.array(solar_in)
    output['q_cab'] = np.array(q_cab)
    output['q_occ'] = np.array(q_occ)
    output['batHeat'] = np.array(batHeat)
    output['q_bat'] = np.array(q_bat)
    output['q_motor'] = np.array(q_motor)
    output['Q_act_evap'] = np.array(Q_act_evap)
    output['Q_act_cond'] = np.array(Q_act_cond)

    output['m_dot'] = np.array(m_dot)
    output['rfgNcomp'] = np.array(rfgNcomp)
    output['cabmdotair'] = np.array(cabmdotair)
    output['condmdotair'] = np.array(condmdotair)
    output['evapmdotair'] = np.array(evapmdotair)
    output['essmdotcool'] = np.array(essmdotcool)
    output['mcmdotcool'] = np.array(mcmdotcool)
    output['essradmdotair'] = np.array(essradmdotair)
    output['mcradmdotair'] = np.array(mcradmdotair)

    output['sumPTMS'] = np.array(sumPTMS)
    output['rfgPcomp'] = np.array(rfgPcomp)
    output['cabPfanair'] = np.array(cabPfanair)
    output['evapPfanair'] = np.array(evapPfanair)
    output['condPfanair'] = np.array(condPfanair)
    output['esscoolPpump'] = np.array(esscoolPpump)
    output['mccoolPpump'] = np.array(mccoolPpump)
    output['essradPfanair'] = np.array(essradPfanair)
    output['mcradPfanair'] = np.array(mcradPfanair)
    output['EER'] = np.array(EER)
    output['operation_mode'] = np.array(operation_mode)

    output['e_cabin'] = np.array(e_cabin)
    output['i_term_cabin'] = np.array(i_term_cabin)
    output['e_battery'] = np.array(e_battery)
    output['i_term_battery'] = np.array(i_term_battery)
    output['e_motor'] = np.array(e_motor)
    output['i_term_motor'] = np.array(i_term_motor)

    output['mcpin'] = np.array(mcElecKwInAch)*1e3
    output['mcpout'] = np.array(mcMechKwOutAch )* 1e3
    output['power'] = np.array(essKwOutAch )* 1e3
